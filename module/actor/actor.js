...
export class MothershipActor extends Actor {
  ...
  prepareDerivedData() {
    ...
    // Cloud Empress: Migrate old Mothership data to Cloud Empress format
    if (this.type === "character") {
      this._migrateToCloudEmpress();
    }
  ...
  // Migrate old Mothership characters to Cloud Empress format
  _migrateToCloudEmpress() {
    const system = this.system;
    let needsUpdate = false;
    const updates = {};

    // Check if we have old stats that need migrating
    if (!system.stats.mind && system.stats.intellect) {
      // Migrate intellect -> mind
      updates["system.stats.mind"] = {
        value: system.stats.intellect.value || 30,
        mod: system.stats.intellect.mod || 0,
        min: 0,
        max: 99,
        label: "Mind",
        rollLabel: "Mind Check"
      };
      needsUpdate = true;
    }

    if (!system.stats.heart && system.stats.combat) {
      // Migrate combat -> heart
      updates["system.stats.heart"] = {
        value: system.stats.combat.value || 30,
        mod: system.stats.combat.mod || 0,
        min: 0,
        max: 99,
        label: "Heart",
        rollLabel: "Heart Check"
      };
      needsUpdate = true;
    }

    if (!system.stats.reality && system.stats.sanity) {
      // Migrate sanity -> reality
      updates["system.stats.reality"] = {
        value: system.stats.sanity.value || 25,
        mod: system.stats.sanity.mod || 0,
        min: 0,
        max: 99,
        label: "Reality",
        rollLabel: "Reality Save"
      };
      needsUpdate = true;
    }

    // Add missing Cloud Empress fields
    if (!system.other?.chalk) {
      updates["system.other.chalk"] = {
        value: 0,
        min: 0,
        max: 99,
        label: "Chalk"
    ...
    if (!system.other?.provisions) {
      updates["system.other.provisions"] = {
        value: 1,
        min: 0,
        max: 10,
        label: "Provisions"
    ...
    // Update stress to 0-10 range if it's in old 2-20 range
    if (system.other?.stress && system.other.stress.max === 20) {
      updates["system.other.stress.max"] = 10;
      updates["system.other.stress.value"] = Math.min(system.other.stress.value, 10);
      needsUpdate = true;
    }
  ...
  _deriveCreature() {
    ...
    // Cloud Empress: Swarms use strength stat for combat capability
    if (system.swarm && system.swarm.enabled){
      system.stats.strength.value = system.swarm.strength.value * ( system.hits.max - system.hits.value );
    }
  }
  ...
  getFlavorText(type, context, action) {
    ...
    //Cloud Empress uses stress system
  ...
  async rollTable(tableId, rollString, aimFor, zeroBased, checkCrit, rollAgainst, comparison) {
    ...
    //customize this roll if its a unique use-case
      ...
      if (tableId === 'panicCheck') {
        ...
        specialRoll = tableId;
        //assign variables - Cloud Empress uses 1e rules
        tableId = game.settings.get('cloudempress', 'table1ePanic');
        aimFor = 'high';
        zeroBased = false;
        checkCrit = false;
        ...
        if (rollString === '[-]' || rollString === '' || rollString === '[+]') {
          rollString = '1d10' + rollString;
        }
      }
      //wound check
      if (tableId === 'woundCheck') {
        //set special roll value for use later
        specialRoll = tableId;
        //assign variables
        tableId = game.settings.get('cloudempress', 'table1eWound');
        aimFor = 'low';
        zeroBased = true;
        checkCrit = false;
        //assign rollString if its a partial
      ...
      //death save
      if (tableId === 'deathSave') {
        //set special roll value for use later
        specialRoll = tableId;
        //assign variables
        tableId = game.settings.get('cloudempress', 'table1eDeath');
        aimFor = 'low';
        zeroBased = true;
        checkCrit = false;
        //assign rollString if its a partial
      ...
      if (tableId === 'maintenanceCheck') {
        ...
        specialRoll = tableId;
        //assign variables
      tableId = game.settings.get('cloudempress', 'table1eMaintenance');
        zeroBased = true;
        checkCrit = true;
    ...
    //bounce this request away if certain parameters are NULL
      ...
      if (!rollString) {
        //Cloud Empress always uses 1d10 for panic checks
        let rollDie = '1d10';
    ...
    if (game.settings.get('cloudempress', 'panicDieTheme') != "") { //We're going to check if the theme field is blank. Otherwise, don't use this.
        //set panic die color
      let dsnTheme = game.settings.get('cloudempress', 'panicDieTheme');
    ...
    //if this is a death save with certain results, we need to roll on curse table
      //roll a second die if needed (curse table for death save results 1-7)
      if (specialRoll === 'deathSave' && parsedRollResult.total >= 1 && parsedRollResult.total <= 7) {
        //determine the rollString
        let rollString2 = '1d10';
        //get the curse table ID and switch to it
        let curseTableId = game.settings.get('cloudempress', 'table1eCurse');
        tableData = await fromIdUuid(curseTableId, {type:"RollTable"});
        console.log('Curse table data:', tableData);
        //update table metadata
        tableName = tableData.name;
        tableImg = tableData.img;
        //roll second dice on curse table
        rollResult2 = await new Roll(rollString2).evaluate();
        //roll second set of dice
      parsedRollResult2 = this.parseRollResult(rollString2, rollResult2, true, false, null, null, 'curseRoll');
        //set marker for HTML
        secondRoll = true;
        //set table result number to curse roll
        tableResultNumber = parsedRollResult2.total;
        //log second die
        console.log(`Death save triggered curse roll: ${parsedRollResult2.total}`);
      }
    ...
    //make any custom changes to chat message
      //Cloud Empress doesn't need panic check customization
    //assign message description text
    if (specialRoll === 'panicCheck') {
      msgDesc = 'Rolling vs Stress to avoid panic...';
      if (parsedRollResult.success) {
        flavorText = 'You keep your cool and avoid panicking.';
        if (parsedRollResult.critical) {
          flavorText = flavorText + ' You relieve 1 Stress.';
        }
      } else {
        flavorText = 'You panic!';
      }
    } else if (specialRoll === 'woundCheck') {
      msgDesc = 'Rolling for wound severity...';
      flavorText = 'You brace for the worst.';
    } else if (specialRoll === 'deathSave') {
      msgDesc = 'Rolling to see if you survive...';
      flavorText = 'You knock on death\'s door.';
    } else {
      msgDesc = 'Rolling on ' + tableName + '...';
      if (parsedRollResult.success) {
        flavorText = 'Success!';
        if (parsedRollResult.critical) {
          flavorText = flavorText + ' Critical success!';
        }
      } else {
        flavorText = '';
      }
    }
      //append effects for Maintenance Check Failure
      if (specialRoll === 'maintenanceCheck' && !parsedRollResult.success && !parsedRollResult.critical) {
        tableResultFooter = `<br><br>Everyone on board the ship takes 1 Stress.<br><br>@UUID[Compendium.cloudempress.macros_triggered_1e.dvJR9DYXI2kV0BbR]{+1 Stress}`;
      }
      //append effects for Critical Maintenance Check Failure
      if (specialRoll === 'maintenanceCheck' && !parsedRollResult.success && parsedRollResult.critical) {
        tableResultFooter = `<br><br>Everyone on board the ship takes 1 Stress. You must roll for another maintenance issue because you critically failed.<br><br>@UUID[Compendium.cloudempress.macros_triggered_1e.dvJR9DYXI2kV0BbR]{+1 Stress}<br><br>@UUID[Compendium.cloudempress.macros_triggered_1e.hRapiXGVW8WZQH12]{Roll for Maintenance Issue}`;
      }
    ...
    if (tableResult[0].type === 0 || tableResult[0].type === 'text') {
      ...
      //prepare template
      messageTemplate = 'systems/cloudempress/templates/chat/rollTable.html';
      //render template
  ...
  async chooseAttribute(rollString, aimFor) {
    ...
    return new Promise(async (resolve) => {
      ...
      //create HTML for this window
        //header
      let dialogDesc = await renderTemplate('systems/cloudempress/templates/dialogs/skill-check-stat-selection-dialog.html');
  ...
  //Cloud Empress: Roll a stat check with advantage when using a skill
  async rollSkillWithAdvantage(skillName) {
    //wrap the whole thing in a promise, so that it waits for the form to be interacted with
    return new Promise(async (resolve) => {
      //create HTML for the stat selection dialog
      let dialogDesc = `
        <div class="macro_window" style="padding-left: 8px;">
          <div class="macro_desc">
            <h4>${game.i18n.localize("Mosh.SelectAStat")}</h4>
            <p>You are using the skill: <strong>${skillName}</strong></p>
            <p>Select which stat to roll with advantage:</p>
          </div>
        </div>
        <label for="mind-stat">
          <div class="macro_window" style="vertical-align: middle; padding-left: 3px;">
            <div class="grid grid-3col" style="align-items: center; grid-template-columns: 20px 60px auto">
              <input type="radio" id="mind-stat" name="stat" value="mind" checked="checked">
              <div class="macro_desc" style="display: table;">
                <span style="display: table-cell; vertical-align: middle;">
                  <strong>${game.i18n.localize("Mosh.Mind")}</strong> (${this.system.stats.mind.value})
                </span>
              </div>
            </div>
          </div>
        </label>
        <label for="heart-stat">
          <div class="macro_window" style="vertical-align: middle; padding-left: 3px;">
            <div class="grid grid-3col" style="align-items: center; grid-template-columns: 20px 60px auto">
              <input type="radio" id="heart-stat" name="stat" value="heart">
              <div class="macro_desc" style="display: table;">
                <span style="display: table-cell; vertical-align: middle;">
                  <strong>${game.i18n.localize("Mosh.Heart")}</strong> (${this.system.stats.heart.value})
                </span>
              </div>
            </div>
          </div>
        </label>
        <label for="strength-stat">
          <div class="macro_window" style="vertical-align: middle; padding-left: 3px;">
            <div class="grid grid-3col" style="align-items: center; grid-template-columns: 20px 60px auto">
              <input type="radio" id="strength-stat" name="stat" value="strength">
              <div class="macro_desc" style="display: table;">
                <span style="display: table-cell; vertical-align: middle;">
                  <strong>${game.i18n.localize("Mosh.Strength")}</strong> (${this.system.stats.strength.value})
                </span>
              </div>
            </div>
          </div>
        </label>
        <label for="speed-stat">
          <div class="macro_window" style="vertical-align: middle; padding-left: 3px;">
            <div class="grid grid-3col" style="align-items: center; grid-template-columns: 20px 60px auto">
              <input type="radio" id="speed-stat" name="stat" value="speed">
              <div class="macro_desc" style="display: table;">
                <span style="display: table-cell; vertical-align: middle;">
                  <strong>${game.i18n.localize("Mosh.Speed")}</strong> (${this.system.stats.speed.value})
                </span>
              </div>
            </div>
          </div>
      ...
      //create final dialog data
      const dialogData = {
        window: {title: `${skillName} - ${game.i18n.localize("Mosh.ChooseAStat")}`},
        classes: ["macro-popup-dialog"],
        position: {width: 500},
        content: dialogDesc,
        buttons: [
          {
            label: game.i18n.localize("Mosh.Roll"),
            action: `action_roll`,
            callback: (event, button, dialog) => {
              const selectedStat = button.form.querySelector("input[name='stat']:checked")?.getAttribute("value");
              if (selectedStat) {
                // Roll with advantage ([+] modifier)
                this.rollCheck(`1d100 [+]`, `low`, selectedStat, skillName, null, null);
              }
              resolve();
            },
  ...
  //Cloud Empress: Roll a weapon attack with stat selection
  async rollWeaponWithStatChoice(weaponItem) {
    //wrap the whole thing in a promise, so that it waits for the form to be interacted with
    return new Promise(async (resolve) => {
      //create HTML for the stat selection dialog
      let dialogDesc = `
        <div class="macro_window" style="padding-left: 8px;">
          <div class="macro_desc">
            <h4>${game.i18n.localize("Mosh.SelectAStat")}</h4>
            <p>You are using: <strong>${weaponItem.name}</strong></p>
            <p>Select which stat to roll:</p>
          </div>
        </div>
        <label for="mind-stat">
          <div class="macro_window" style="vertical-align: middle; padding-left: 3px;">
            <div class="grid grid-3col" style="align-items: center; grid-template-columns: 20px 60px auto">
              <input type="radio" id="mind-stat" name="stat" value="mind">
              <div class="macro_desc" style="display: table;">
                <span style="display: table-cell; vertical-align: middle;">
                  <strong>${game.i18n.localize("Mosh.Mind")}</strong> (${this.system.stats.mind.value})
                </span>
        ...
        <label for="strength-stat">
          <div class="macro_window" style="vertical-align: middle; padding-left: 3px;">
            <div class="grid grid-3col" style="align-items: center; grid-template-columns: 20px 60px auto">
              <input type="radio" id="strength-stat" name="stat" value="strength" checked="checked">
              <div class="macro_desc" style="display: table;">
                <span style="display: table-cell; vertical-align: middle;">
                  <strong>${game.i18n.localize("Mosh.Strength")}</strong> (${this.system.stats.strength.value})
                </span>
      ...
      //create final dialog data
      const dialogData = {
        window: {title: `${weaponItem.name} - ${game.i18n.localize("Mosh.ChooseAStat")}`},
        classes: ["macro-popup-dialog"],
        position: {width: 500},
        content: dialogDesc,
        buttons: [
          {
            ...
            callback: (event, button, dialog) => {
              const selectedStat = button.form.querySelector("input[name='stat']:checked")?.getAttribute("value");
              if (selectedStat) {
                // Roll weapon attack without advantage
                this.rollCheck(null, 'low', selectedStat, 'none', 0, weaponItem);
              }
              resolve();
            },
            icon: `fas fa-dice-d20`
  ...
  async chooseSkill(dlgTitle, rollString) {
    ...
    return new Promise(async (resolve) => {
      ...
      //create HTML for this window
        //header
        let skillHeader = await renderTemplate('systems/cloudempress/templates/dialogs/choose-skill-dialog-header.html');
        //skill template
        let skillRow = `
        <label for="[RADIO_ID]">
        <div class ="macro_window" style="vertical-align: middle; padding-left: 3px;">
  ...
  async rollCheck(rollString, aimFor, attribute, skill, skillValue, weapon,overrideDamagaRollString=null) {
    ...
    //customize this roll if its a unique use-case
      ...
      if (attribute === 'damage') {  
        ...
        checkCrit = false;
        //set attribute (Cloud Empress: Use strength for melee, speed for ranged)
        attribute = weapon && weapon.system.range > 0 ? 'speed' : 'strength';
        //set skill + value
        skill = 'none';
        skillValue = 0;
        //set rollstring
      ...
      if (attribute === 'restSave') {
        ...
        //lets figure out the actors worst save and update this roll accordingly
          ...
          let realitySave = Number(this.system.stats.reality.value) + Number(this.system.stats.reality.mod || 0);
    ...
    if (specialRoll === 'damage') {  
      ...
      let dsnTheme = 0;
      if (game.settings.get('cloudempress', 'damageDiceTheme') != "") { //We're going to check if the theme field is blank. Otherwise, don't use this.
        //set damage dice color
        dsnTheme = game.settings.get('cloudempress', 'damageDiceTheme');
      }
      //prepare flavortext
      ...
      //generate chat message
        ...
        //create message
        const template = 'systems/cloudempress/templates/chat/rollCheck.html';
    ...
    //set chat message text
      ...
      if (game.settings.get('cloudempress', 'damageDiceTheme') != "") { //We're going to check if the theme field is blank. Otherwise, don't use this.
        ...
        if (game.settings.get('cloudempress', 'critDamage') === 'advantage') {
            parsedDamageString = '{' + parsedDamageString + ',' + parsedDamageString + '}kh';
        } else if (game.settings.get('cloudempress', 'critDamage') === 'doubleDamage') {
            critMod = ' * 2';
        } else if (game.settings.get('cloudempress', 'critDamage') === 'doubleDice') {
            critMod = ' + ' + parsedDamageString + '[' + dsnTheme + ']';
        } else if (game.settings.get('cloudempress', 'critDamage') === 'maxDamage') {
          parsedDamageString = parsedDamageString.replaceAll('d', ' * ');
        } else if (game.settings.get('cloudempress', 'critDamage') === 'weaponValue') {
            critMod = ' + ' + weapon.system.critDmg + '[' + dsnTheme + ']';
        } else if (game.settings.get('cloudempress', 'critDamage') === 'none') {
            //do nothing
          }
        }
        ...
        if (parsedRollResult.success === true) {
          ...
          if (game.settings.get('cloudempress', 'autoStress')) { //If the automatic stress option is enabled
      ...
      //prepare flavor text for special rolls
        ...
        if (specialRoll === 'restSave') {
          ...
          //override  header image
          msgImgPath = `systems/cloudempress/images/icons/ui/macros/rest_save.png`;
          //prepare attribute label
          attributeLabel = this.system.stats[attribute].label;
        ...
        if (specialRoll === 'bankruptcySave') {
          ...
          //set header image
          msgImgPath = 'systems/cloudempress/images/icons/ui/rolltables/bankruptcy_save.png';
          //prepare attribute label
          attributeLabel = game.i18n.localize("Mosh.Bankrupcy");
          //get the bankruptcy table
          let tableId = game.settings.get('cloudempress','table1eBankruptcy');
          //get Table Data
          let tableData = await fromIdUuid(tableId,{type:"RollTable"});
        ...
        if (specialRoll === 'moraleCheck') {
          ...
          //set header image
          msgImgPath = 'systems/cloudempress/images/icons/ui/macros/morale_check.png';
          //prepare attribute label
      ...
      //prepare flavor text for regular checks
        ...
        //set header image
        msgImgPath = 'systems/cloudempress/images/icons/ui/attributes/' + attribute + '.png';
        //prep text based on success or failure
        if (parsedRollResult.success === false && this.type === 'character') {
          if (game.settings.get('cloudempress', 'autoStress')) { //If the automatic stress option is enabled
            //increase stress by 1 and retrieve the flavor text from the result
      ...
      //prepare template
      messageTemplate = 'systems/cloudempress/templates/chat/rollCheck.html';
      //render template
      messageContent = await renderTemplate(messageTemplate, messageData);
      //make message
      let macroMsg = await rollResult.toMessage({
    ...
    //calculate the change, whether from a value, roll (can only be one, it will check modValue first)
      ...
      if (modifyMaximum || modifyMaximum === 0) {
        if (modifyNew > modifyMaximum) {
          ...
          //if health reaches maximum, trigger wound and reset to 0
          if (fieldId === 'health' && modifyNew >= modifyMaximum && this.system.hits.value < this.system.hits.max) {
        ...
        //create modification text (for chat message or return values)
          //get flavor text
          if (modifyChange > 0) {
            //special handling for wounds to avoid broken macro links
            if (fieldId === 'hits') {
              msgFlavor = '';
              msgChange = 'increased';
              msgHeader = fieldPrefix + 'Wounded';
              msgImgPath = 'systems/cloudempress/images/icons/ui/attributes/health.png';
            } else {
            ...
            //special handling for wounds
            if (fieldId === 'hits') {
              msgFlavor = '';
              msgChange = 'decreased';
              msgHeader = fieldPrefix + 'Mended';
              msgImgPath = 'systems/cloudempress/images/icons/ui/attributes/health.png';
            } else {
        ...
        //prepare flavor text
          //set message outcome for health reaches maximum, and you have wounds remaining
          if (getWound) {
            //can this player take a wound and not die?
            if (this.system.hits.value === this.system.hits.max) {
              //you are dead!
          msgOutcome = 'The end draws near...';
            } else if (this.system.hits.value + 1 === this.system.hits.max) {
              //you are wounded!
          msgOutcome = 'Your health has reached maximum and you must take a wound. Your health has been reset to 0.';
            } else {
              //you are wounded!
          msgOutcome = 'Your health has reached maximum and you must take a wound. Your health has been reset to 0.';
            }
          //set message outcome for past ceiling or floor
          } else if (msgAction === 'pastCeiling' && fieldId === 'hits') {
        msgOutcome = 'The end draws near . . .';
        ...
        msgOutcome = this.getFlavorText('attribute', fieldId, msgAction) + ` <strong>Reduce the most relevant Stat or Save by ${modifySurplus}</strong>.`;
          ...
          } else if (fieldId === 'hits' && msgAction === 'hitCeiling') {
            //at maximum wounds
        msgOutcome = 'You have reached maximum wounds.';
          } else if (fieldId === 'hits') {
            //wounds changed
        msgOutcome = fieldPrefix + fieldLabel.reduce((a, v) => a[v], this) + ` ` + msgChange + ` from <strong>${modifyCurrent}</strong> to <strong>${modifyNew}</strong>.`;
          } else {
        ...
        if (outputChatMsg) {
          //generate chat message
            ...
            //prepare template
            messageTemplate = 'systems/cloudempress/templates/chat/modifyActor.html';
            //render template
            messageContent = await renderTemplate(messageTemplate, messageData);
            //push message
            ChatMessage.create({
  ...
  async modifyItem(itemId, addAmount) {
    ...
    if (this.items.getName(itemData.name)) {
      ...
      //if this is a weapon or armor, add another one
        ...
        // Handle all other item types when actor already has one (ability, spell, crest, spore, etc.)
        await this.createEmbeddedDocuments('Item', [itemData]);
        flavorText = game.i18n.localize("Mosh.YouAddAnotherOfThisToYourInventory");
      }
    } else {
      ...
      //if this is a weapon or armor, add it
        ...
        // Handle all other item types (ability, spell, crest, spore, etc.)
        await this.createEmbeddedDocuments('Item', [itemData]);
        flavorText = game.i18n.localize("Mosh.YouAddThisToYourInventory");
      }
    }
    //generate chat message
      ...
      //prepare template
      messageTemplate = 'systems/cloudempress/templates/chat/modifyItem.html';
      //render template
      messageContent = await renderTemplate(messageTemplate, messageData);
      //make message
      ChatMessage.create({
  ...
  async reloadWeapon(itemId) {
    ...
    //generate chat message
      ...
      //prepare template
      messageTemplate = 'systems/cloudempress/templates/chat/reload.html';
      //render template
      messageContent = await renderTemplate(messageTemplate, messageData);
      //push message
      ChatMessage.create({
  ...
  async takeRadiationDamage() {
    ...
    this.modifyActor('system.stats.mind.value', -1, null, false);
    this.modifyActor('system.stats.heart.value', -1, null, false);
    this.modifyActor('system.stats.reality.value', -1, null, false);
  ...
  async takeCryoDamage(rollString) {
    ...
    this.modifyActor('system.stats.mind.value', parsedRollResult.total, null, false);
    this.modifyActor('system.stats.heart.value', parsedRollResult.total, null, false);
    this.modifyActor('system.stats.reality.value', parsedRollResult.total, null, false);
    ...
    <div class="mosh">
      <div class="rollcontainer">
          ...
          <div style="text-align: right"><img class="roll-image" src="systems/cloudempress/images/icons/ui/attributes/health.png" /></div>
          </div>
          <div class="description"" style="margin-bottom: 20px;">
          <div class="body">
          ${msgFlavor}
  ...
  async chooseCover() {
    ...
    return new Promise(async (resolve) => {
      ...
      //create pop-up HTML
      let msgContent = await renderTemplate('systems/cloudempress/templates/dialogs/choose-cover-dialog.html', {
          curDR:curDR, 
          curAP:curAP, 
  ...
  //Cloud Empress: Cast a spell
  async castSpell(spell) {
    // Check chalk
    const chalkCost = spell.system.chalkCost || 1;
    const currentChalk = this.system.other.chalk.value;

    if (currentChalk < chalkCost) {
      ui.notifications.warn("Not enough chalk to cast this spell!");
      return;
    }

    // Consume chalk
    await this.update({
      "system.other.chalk.value": currentChalk - chalkCost
    });

    // Add stress
    const currentStress = this.system.other.stress.value;
    await this.update({
      "system.other.stress.value": Math.min(currentStress + 1, 10)
    });

    // Heart check
    const heartRoll = await new Roll("1d100").evaluate();
    const heartValue = this.system.stats.heart.value;
    const success = heartRoll.total <= heartValue;
    ...
    const chatData = {
      ...
      content: success ?
        `<p>Heart Check: ${heartRoll.total} vs ${heartValue} - Success!</p><p>${spell.system.effect}</p><p><em>After-effect: ${spell.system.afterEffect}</em></p>` :
        `<p>Heart Check: ${heartRoll.total} vs ${heartValue} - Failed! Roll on Miscast Table.</p>`
    };
  ...
  async distressSignal() {
    ...
    return new Promise(async (resolve) => {
      //create pop-up HTML
      let msgContent = await renderTemplate('systems/cloudempress/templates/dialogs/distres-signal-dialog.html');
        ...
        buttons: [
          {
            label: game.i18n.localize("Mosh.Advantage"),
			      action: `action_advantage`,
            callback: () => this.rollTable(game.settings.get('cloudempress', 'table1eDistressSignal'), `1d10 [+]`, `low`, true, false, null, null),
            icon: `fas fa-angle-double-up`
          },
          {
            label: game.i18n.localize("Mosh.Normal"),
  ...
  async chatDesc(item) {
    ...
    const template = 'systems/cloudempress/templates/chat/itemRoll.html';
