...
import { MothershipAgeSheet } from "./item/age-sheet.js";
import { MothershipSkillSheet } from "./item/skill-sheet.js";
import { MothershipSpellSheet } from "./item/spell-sheet.js";
import { MothershipCrestSheet } from "./item/crest-sheet.js";
import { MothershipSporeSheet } from "./item/spore-sheet.js";
...
Hooks.once('init', async function () {
  ...
  // Also register under game.cloudempress for Cloud Empress system
  game.cloudempress = {
    MothershipActor,
    MothershipItem,
    rollItemMacro,
...
function rollItemMacro(itemName) {
  ...
  if (game.settings.get('cloudempress','macroTarget') === 'character') {
    ...
    if (!game.user.character) {
      ...
      //run the function for the player's 'Selected Character'
        ...
        //roll action
        if (item.type == "weapon") {
          // Cloud Empress: Use strength for melee, speed for ranged weapons
          const weaponStat = item.system.range > 0 ? 'speed' : 'strength';
          return game.user.character.rollCheck(null, 'low', weaponStat, null, null, item);
  ...
  } else if (game.settings.get('cloudempress','macroTarget') === 'token') {
    //is there a selected character? warn if no
    if (!canvas.tokens.controlled.length) {
      ...
      canvas.tokens.controlled.forEach(function(token){
        ...
        if (item.type == "weapon") {
          // Cloud Empress: Use strength for melee, speed for ranged weapons
          const weaponStat = item.system.range > 0 ? 'speed' : 'strength';
          return token.actor.rollCheck(null, 'low', weaponStat, null, null, item);
        } else if (item.type == "item" || item.type == "armor" || item.type == "ability" || item.type == "condition" || item.type == "repair") {
